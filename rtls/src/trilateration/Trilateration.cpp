 ...
 */
int trilateration(vec3d * const result1, vec3d * const result2, vec3d * const best_solution, const vec3d p1, const double r1, const vec3d p2, const double r2, const vec3d p3, const double r3,
		const vec3d p4, const double r4, const double maxzero) {
 ...
 */
int deca_3dlocate(vec3d * const solution1, vec3d * const solution2, vec3d * const best_solution, int * const nosolution_count, double * const best_3derror, double * const best_gdoprate, vec3d p1,
		double r1, vec3d p2, double r2, vec3d p3, double r3, vec3d p4, double r4, int *combination) {
	...
	*best_gdoprate = 1; /* put the worst gdoprate init */
	...
	do {
		...
		//printf("trilateration success %d\n", success);
		if (success) {
			switch (result) {
			case TRIL_3SPHERES:
				*solution1 = o1;
			...
			case TRIL_4SPHERES:
				...
				if (gdoprate_compare1 <= gdoprate_compare2) {
					...
					*nosolution_count = overlook_count;
					*best_3derror = sqrt(
							(vnorm(vdiff(solution, p1)) - r1) * (vnorm(vdiff(solution, p1)) - r1) + (vnorm(vdiff(solution, p2)) - r2) * (vnorm(vdiff(solution, p2)) - r2)
									+ (vnorm(vdiff(solution, p3)) - r3) * (vnorm(vdiff(solution, p3)) - r3) + (vnorm(vdiff(solution, p4)) - r4) * (vnorm(vdiff(solution, p4)) - r4));
...
int GetLocation(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray) {
	...
	//printf("radius(%.2f, %.2f, %.2f, %.2f)\n", r1, r2, r3, r4);

	//r4 = r1;

	/* get the best location using 3 or 4 spheres and keep it as know_best_location */
	result = deca_3dlocate(&o1, &o2, best_solution, &error, &best_3derror, &best_gdoprate, p1, r1, p2, r2, p3, r3, p4, r1, &combination);

	//cout << "GetLocation " << result << ", sol1: " << o1.x << o1.y << o1.z << ", sol2: " << o2.x << o2.y << o2.z<<endl;
